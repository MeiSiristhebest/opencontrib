...
export function qualifyIssue(input: QualifyIssueInput): QualificationResult {
  ...
  let claimantDetails = "";
  // `now` is injected (see QualifyIssueInput.now) — defaulting to the wall clock
  // only when the caller (production) omits it.
  ...
  const bodyHasAuthorIntent = authorFixPhrases.some((phrase) =>
