...
export function buildTurnPrompt(ctx: PromptRebuildContext): string {
  ...
  if (ctx.feedback.length > 0) {
    lines.push(
      `## Previous Attempt Feedback (Attempts 1-${ctx.attemptNumber - 1})`,
    );
    lines.push("");
    for (const entry of ctx.feedback) {
      const status = entry.success
        ? "✅ PASS"
        : `❌ FAIL (exit ${entry.exitCode ?? "N/A"})`;
  ...
  if (ctx.attemptNumber > 1) {
    lines.push(
      `## Repair Instructions (Attempt ${ctx.attemptNumber}/${ctx.maxAttempts})`,
    );
    lines.push("");
    lines.push(
      `You have ${ctx.maxAttempts - ctx.attemptNumber} attempt(s) remaining.`,
    );
    lines.push(
      "Based on the feedback above, diagnose the root cause and produce a revised surgical patch.",
    );
    lines.push(
      "Focus on the specific failure rather than making unrelated changes.",
    );
    lines.push("");
...
export class AgentOrchestrator {
  ...
  constructor(options: AgentOrchestratorOptions = {}) {
    ...
    const stateMachine =
      options.deps?.stateMachine ??
      new ContributionStateMachine(options.policy);
    const clock = options.deps?.clock ?? new SystemClock();
    const runManager = options.deps?.runManager ?? new ContributionRunManager();
    const llmService =
      options.deps?.llmService ?? resolveLlmService(options.llmService);
  ...
  async runPipeline(input: {
    ...
    if (this.isRunning) {
      return {
        ...
        stage: "BLOCKED",
        reportSummary:
          "Orchestrator already in progress; instantiate a new AgentOrchestrator for concurrent runs.",
  ...
  private async _runPipeline(input: {
    ...
    return {
      ...
      stage: "COMPLETED",
      reportSummary:
        "Pipeline terminated without producing a terminal result (unexpected).",
    };
  }
}
