...
export class AutonomousPoCVerifier {
  ...
  public static async verifyFinding(
    ...
    const findingUri =
      finding.uri || `ptr://${finding.namespace}/${finding.id}`;
    ...
    try {
      ...
      const step: VerificationStep =
        ...
        AutonomousPoCVerifier.synthesizeStep(finding);
      const defaultCmd = AutonomousPoCVerifier.resolveDefaultTestCommand(
        finding.file,
      );

      // Write exploit/harness code if provided
      if (step.setupCode) {
        const ext = finding.file.endsWith(".py")
          ? ".py"
      ...
      const preFixCmd =
        step.invocationExpression || options.testCommand || defaultCmd;
      ...
      const preMatched =
        ...
        Boolean(
          step.expectedFailureAssertion &&
          report.logs.preFixOutput.includes(step.expectedFailureAssertion),
        );
      ...
      if (patch && finding.file) {
        ...
        if (finding.callSite && fullContent.includes(finding.callSite)) {
          report.fixApplied = sandbox.applyPatch(
            finding.file,
            finding.callSite,
            patch,
          );
      ...
      const postMatched =
        ...
        Boolean(
          step.expectedPostFixAssertion &&
          report.logs.postFixOutput.includes(step.expectedPostFixAssertion),
        );
      ...
      if (!regCmd || !regCmd.trim()) {
        report.status = "SKIPPED";
        report.logs.regressionOutput =
          "Regression test skipped: empty test command";
        sandbox.cleanup();
        return report;
      }
