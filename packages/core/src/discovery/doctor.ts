...
let cachedDoctorReport: { report: DoctorReport; expiresAt: number } | null =
  null;
...
export function runDoctorAudit(forceRefresh = false): DoctorReport {
  ...
  if (
    !forceRefresh &&
    cachedDoctorReport &&
    cachedDoctorReport.expiresAt > now
  ) {
  ...
  if (isWindows) {
    if (isBinaryOnPath("git")) {
      ...
      try {
        ...
        // best-effort: a git that resolves on PATH but fails --version is
        // reported as version-unknown, not a hard diagnostic failure
      }
    } else {
      ...
      const programFilesX86 =
        ...
        join(sysDrive, "Program Files (x86)");
      ...
      for (const p of candidatePaths) {
        if (existsSync(p)) {
          ...
          try {
            ...
            // best-effort candidate probe: move on to the next candidate path
          }
        }
      }
    }
    ...
    try {
      ...
      // best-effort: non-Windows git version probe failure is non-fatal
    }
  }
  ...
  try {
    ...
    if (dockerAvailable) {
      checks.push({
        ...
        impact:
          "已就绪：已启用 Tier-1 容器化全隔离沙箱，全套多语言分析工具在容器内开箱即用。",
      });
    } else {
      checks.push({
        category: "Sandbox",
        ...
        impact:
          "无法使用 Tier-1 容器化隔离沙箱；无法自动在预置镜像中免安装运行多语言分析器。",
        contingencyPlan: {
          activePlan:
            "预案 B：自动降级为【原生 Git Worktree 隔离沙箱】+ 探测宿主机本地 %PATH% 编译器/分析器",
          alternatives: [
          ...
          remediationAction:
            "启动 Docker Desktop，或运行 `opencontrib workspace` 管理原生沙箱",
        },
      });
    }
  } catch {
  ...
  if (isWindows) {
    try {
      ...
      checks.push({
        ...
        contingencyPlan: wslAvailable
          ? undefined
          : {
              activePlan:
                "预案 B：在 Windows 原生 PowerShell 环境下运行跨平台兼容测试",
              alternatives: [
                "在 Windows 启用 WSL2 并安装 Ubuntu",
                "使用纯跨平台 Node/Bun 测试",
              ],
  ...
  > = {
    semgrep: {
      impact:
        "缺少本地全局 semgrep 二进制，无法直接本地离线扫描 CWE 污染流与 OWASP Top 10 安全缺陷。",
      activePlan:
        "预案 C：自动降级为【uvx semgrep 一次性免安装调度】或内置纯 TS 启发式 AST 规则。",
      alternatives: [
    ...
    knip: {
      ...
      activePlan:
        "预案 C：自动降级为【npx knip / bun x knip 一次性免安装运行】。",
      alternatives: [
    ...
    ruff: {
      impact:
        "缺少本地全局 ruff 二进制，无法直接本地离线执行极速 Python Linter 检查。",
    ...
    php: {
      impact:
        "缺少 PHP 编译器与 Composer，无法本地运行 PHP 项目的单测与语法诊断。",
      activePlan:
        "预案 A：非 PHP 仓库自动跳过；PHP 仓库使用通用跨语言语法分析。",
    ...
    ruby: {
      ...
      activePlan:
        "预案 A：非 Ruby 仓库自动跳过；Ruby 仓库使用通用跨语言语法分析。",
  ...
  for (const tc of toolchains) {
    ...
    const availableViaDocker =
      !binaryAvailable && dockerAvailable && isDockerSupported;
    ...
    if (binaryAvailable) {
      ...
      checks.push({
        ...
        contingencyPlan: {
          activePlan:
            meta?.activePlan ||
            `预案 C：尝试使用一次性免安装命令或内置纯 TypeScript 启发式规则扫描`,
          ...
          remediationAction:
            meta?.remedy || `启动 Docker Desktop 或安装 ${tc.name}`,
        },
      });
    }
  }
  ...
  const disabledPlugins = Object.entries(allStates).filter(
    ([, s]) => !s.enabled,
  );
  if (disabledPlugins.length > 0) {
    const disabledList = disabledPlugins
      .map(([id, s]) => `${id} (${s.disabledReason || "user"})`)
      .join(", ");
    checks.push({
      category: "Plugins",
