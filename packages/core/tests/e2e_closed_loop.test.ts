...
describe("End-to-End Autonomous Contribution Closed Loop & Permission Sandboxing", () => {
  ...
  afterEach(() => {
    if (fs.existsSync(tempDir)) {
      try {
        ...
        console.warn(
          `[e2e-cleanup] Could not remove temp dir "${tempDir}": ${err.message}`,
        );
      }
    }
  });
  ...
  it("enforces runtime permission boundaries strictly (P0 Permission Sandbox)", async () => {
    ...
    expect(host.registerPlugin(restrictedPlugin)).rejects.toThrow(
      PluginPermissionError,
    );
  });
...
`;
    ...
    const securityAstProbe: OpenContribPlugin = {
      ...
      activate: (ctx) => {
        ctx.probes.register({
          ...
          description:
            "Detects unsanitized path.join calls that can escape root",
            ...
            if (fs.existsSync(fileUtilsPath)) {
              ...
              if (content.includes("path.join(baseDir, userPath)")) {
                pointers.create({
                  ...
                  dataFlow:
                    "userPath -> path.join(baseDir, userPath) -> unescaped return",
                  slice: {
                    codeSnippet: "return path.join(baseDir, userPath);",
                    surroundingContext: content,
                    ruleExplanation:
                      "Unsanitized userPath concatenation allows directory traversal via ../ escape.",
                    remediationSuggestion:
                      "Validate that resolved path starts with baseDir path prefix.",
                  },
                  evidence: {
                    ...
                    verificationSteps: [
                      {
                        ...
                        expectedPostFixAssertion:
                          "throws Error or remains bounded within base",
                      },
                    ],
                  },
                });
    ...
    const scannerProbe = selectedProbes.find(
      (p) => p.id === "path-traversal-scanner",
    );
...
`;
    ...
    expect(() => safeResolver("/tmp/safe", "../../../etc/passwd")).toThrow(
      "Path traversal attempt detected",
    );
    expect(safeResolver("/tmp/safe", "sub/file.txt")).toBe(
      path.resolve("/tmp/safe", "sub/file.txt"),
    );
