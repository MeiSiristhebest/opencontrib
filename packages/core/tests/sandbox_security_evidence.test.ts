...
describe("Workspace Security Boundaries & Path Traversal Protection", () => {
  ...
  test("detects and blocks path traversal attempts escaping root", () => {
    ...
    expect(
      manager.isPathWithinWorkspace(testRoot, "deep/nested/folder/file.js"),
    ).toBe(true);

    // Traversal attacks
    expect(manager.isPathWithinWorkspace(testRoot, "../outside.ts")).toBe(
      false,
    );
    expect(manager.isPathWithinWorkspace(testRoot, "../../etc/passwd")).toBe(
      false,
    );
  });
  ...
  test("applySurgicalFilesSafely strictly enforces file count and size limits", () => {
    ...
    const countResult = manager.applySurgicalFilesSafely(
      testRoot,
      excessiveFiles,
    );
...
describe("LLM Service Engineering & Provider Separation", () => {
  ...
  test("MockLLMProvider functions reliably for offline test suites", async () => {
    ...
    const response = await mock.complete(
      "Please generate a surgical patch with PatchDraftSchema",
    );
...
describe("Evidence-Backed Quality Rubric & Subagent Review Decoupling", () => {
  ...
  test(
    "WorktreeManager strictly fails closed when cloning a nonexistent repository fails",
    () => {
      const manager = new WorktreeManager();
      expect(() => {
...
+`;
  ...
  test("TestOutputParserRegistry parses multiple ecosystem outputs and supports custom extension (OCP)", async () => {
    const { defaultTestOutputParserRegistry, TestOutputParserRegistry } =
  ...
  test("countAddedTestCasesFromGitDiff uses injected VcsDeltaPort (DIP)", async () => {
    ...
    const count = await countAddedTestCasesFromGitDiff(
      "/fake/cwd",
      "mock-sha",
      mockVcsAdapter,
    );
