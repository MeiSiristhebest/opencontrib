...
describe("Capability Router, Scoring Engine & Evidence Graph", () => {
  ...
  it("scores and routes capabilities dynamically based on repo fingerprint", async () => {
    ...
    const selectedProviderIds = plan.selectedCapabilities.map(
    ...
    const deferredIds = plan.deferredHeavyCapabilities.map(
      (c) => c.provider.providerId,
    );
    expect(deferredIds).toContain("codeql-deep");

    // When enableHeavy is true, CodeQL is promoted to selected
    const heavyPlan = host.router.planRouting(goFingerprint, {
      intent: "deep_security",
      enableHeavy: true,
    });
    const heavySelectedIds = heavyPlan.selectedCapabilities.map(
      (c) => c.provider.providerId,
    );
  ...
  it("links and retrieves complete relational Evidence Chains in EvidenceGraph", () => {
    ...
    if (!("error" in chain)) {
      expect(chain.findingUri).toMatch(/^ptr:\/\/findings\/v2-[0-9a-f]{64}$/);
      expect(chain.hotspotUri).toMatch(/^ptr:\/\/hotspots\/v2-[0-9a-f]{64}$/);
      expect(chain.pocUri).toMatch(/^ptr:\/\/poc\/v2-[0-9a-f]{64}$/);
