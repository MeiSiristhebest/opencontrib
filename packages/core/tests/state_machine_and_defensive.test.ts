    ...
    const summary: ContributionRunSummary = {
      ...
      artifacts: {
        ...
        evidence: {
          redEvidence: {
            command: "bun test",
            expectedAssertion: "expected failure marker",
            observedOutputSnippet: "FAILED expected failure marker",
            exitCode: 1,
...
`;
    ...
    expect(
      res.crossPlatformHazards.some((h) => h.includes("filepath.ToSlash")),
    ).toBe(true);
  });
...
`;
    ...
    expect(report.failedTests[0].testName).toBe(
    ...
    expect(report.failedTests[0].failureMessage).toContain(
      "expected traversal error",
    );
    expect(report.rootCauseSummary).toContain(
      "TestCodeSearch_RejectsBackslashPathTraversal",
    );
  });
...
describe("Resilient Sandbox Runner & Targeted Package Resolver", () => {
  ...
  it("sanitizes Windows -race command when GCC is absent", () => {
    const sanitized = sanitizeTestCommand(
      "go test",
      ["-race", "./internal/diff/..."],
      true,
    );
