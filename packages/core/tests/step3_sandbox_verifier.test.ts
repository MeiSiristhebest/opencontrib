...
describe("Step 3: Clean-Room Worktree Sandbox, PoC Verifier & Issue Claim Protocol", () => {
  ...
  beforeEach(() => {
    tempRepo = fs.mkdtempSync(
      path.join(os.tmpdir(), "opencontrib-step3-test-"),
    );
  });
  ...
  it("manages Clean-Room Worktree Sandbox lifecycle safely", () => {
    ...
    expect(fs.readFileSync(path.join(tempRepo, "sample.ts"), "utf8")).toContain(
      "10",
    );
  ...
  it("generates authoritative claim statement and discriminates bot accounts", () => {
    const claim = IssueClaimEngine.generateClaimPayload(
      105,
      "Memory leak in Goroutine Pool",
      {
        namespace: "findings",
        id: "leak-1",
    ...
    expect(claim.claimComment).toContain(
      "I am investigating this issue and will share a reproduction once it is verified.",
    );
    expect(claim.claimComment).toContain(
      "Please assign this issue to me if appropriate; no fix or PR submission is being claimed yet.",
    );
    expect(claim.claimComment).toContain("client.go:42");
    expect(claim.isReadyForPR).toBe(false);

    // Bot discrimination checks
    expect(IssueClaimEngine.isBotAuthor("github-actions[bot]", "Bot")).toBe(
      true,
    );
