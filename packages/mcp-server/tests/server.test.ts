...
describe("OpenContrib MCP Server", () => {
  it("initializes and registers all 35 composable contribution domain tools, resources, and workflow prompt", () => {
...
describe("OpenContrib MCP Installer", () => {
  it("generates client configuration with the published npm package name", () => {
    ...
    expect(npxConfig.mcpServers.opencontrib.args).toEqual([
      "-y",
      "@opencontrib/mcp",
    ]);
    expect(bunxConfig.mcpServers.opencontrib.args).toEqual([
      "@opencontrib/mcp",
    ]);
  });
  ...
  it("writes client configuration with the published npm package name", () => {
    ...
    try {
      ...
      expect(config.mcpServers.opencontrib.args).toEqual([
        "-y",
        "@opencontrib/mcp",
      ]);
    } finally {
