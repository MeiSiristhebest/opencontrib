...
program
  ...
  .hook("preAction", (thisCommand, actionCommand) => {
    if (
      (thisCommand.opts() as any).home &&
      typeof (thisCommand.opts() as any).home === "string"
    ) {
  ...
  .configureHelp({
    subcommandTerm: (cmd) =>
      cmd.name() + ((cmd.options as any[]).length ? " [options]" : ""),
...
// Deprecated MCP passthrough for backward compatibility
program
  .command("mcp [subcommand...]")
  .description(
    "(Deprecated) MCP server entrypoint passthrough. Prefer `npx -y @opencontrib/mcp` directly.",
  )
  .allowUnknownOption(true)
  .action(async (subargs: string[]) => {
    console.warn(
      "⚠️  Deprecated: `@opencontrib/cli mcp` is deprecated and will be removed in a future release.",
    );
    console.warn(
      "⚠️  Please use `@opencontrib/mcp` directly (e.g., `npx -y @opencontrib/mcp setup --all` or `npx -y @opencontrib/mcp`).",
    );
    try {
      const { spawn } = await import("node:child_process");
      const child = spawn(
        "npx",
        ["-y", "@opencontrib/mcp", ...(subargs || [])],
        {
          stdio: "inherit",
      ...
      console.error(`Failed to forward MCP command: ${err.message}`);
