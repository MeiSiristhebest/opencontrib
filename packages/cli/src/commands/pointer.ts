...
export const pointerCommand = new Command("pointer")
    .alias("ptr")
    .description(
        "Interact with and dereference OpenContrib Smart Pointers (ptr://...)",
    );

pointerCommand
    .command("list [namespace]")
    .description("List all available smart pointers (Level 1 stub metadata)")
    .option("--dir <path>", "Workspace directory to resolve pointers from")
    .option("--pretty", "Pretty-print JSON output", false)
    .action((namespace, opts) => {
        try {
            const store = new SmartPointerStore({ workspacePath: opts.dir });
...
pointerCommand
    .command("resolve <uri>")
    .description(
        "Dereference a smart pointer with progressive views: stub (Level 1), slice (Level 2), evidence (Level 3)",
    )
    .option("--dir <path>", "Workspace directory to resolve pointers from")
    .option(
        "--view <view>",
        "Dereferencing view: stub, slice, evidence, all",
        "slice",
    )
    .option("--pretty", "Pretty-print JSON output", false)
    .action((uri, opts) => {
