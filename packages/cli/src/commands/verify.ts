...
export const verifyCommand = new Command("verify")
  .description(
    "Verify a discovered finding pointer in an isolated clean-room worktree sandbox",
  )
  .argument(
    "<pointerUri>",
    "Smart Pointer URI to verify (e.g. ptr://findings/...)",
  )
  ...
  .action(async (pointerUri: string, opts) => {
    try {
      ...
      const targetFinding: PointerStub =
        stub && stub.id
          ? stub
          : {
              namespace: "findings",
