...
export const setupCommand = new Command("setup")
  ...
  .option(
    "--only <tools>",
    "Only check/install specified tool IDs (comma-separated)",
  )
  .option("--pretty", "Pretty-print output", false)
  .action(
    async (opts: { install?: boolean; only?: string; pretty?: boolean }) => {
      try {
        ...
        const results = TOOL_REGISTRY.filter((t) => toolIds.includes(t.id)).map(
          (entry) => {
          ...
          for (const tool of missing) {
            if (!tool.installSteps || tool.installSteps.length === 0) {
              printJSON(
                {
                  status: "error",
                  message: `No install method for ${tool.id}`,
                },
                opts.pretty,
              );
              failedCount++;
        ...
        const result: SetupResult = {
          status:
            failedCount > 0 && installedCount === 0
              ? "error"
              : failedCount > 0
                ? "partial"
                : "success",
