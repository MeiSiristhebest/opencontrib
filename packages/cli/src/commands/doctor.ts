...
export const doctorCommand = new Command("doctor")
  .description(
    "Audit host environment health (Git, Node/Bun, Docker, WSL, storage)",
  )
  ...
  .action(async (opts: { pretty?: boolean; json?: boolean }) => {
    try {
      ...
      if (useTable) {
        ...
        console.log(
          `  Environment: ${report.environment.os} | Node ${report.environment.nodeVersion} | Bun ${report.environment.bunVersion || "N/A"}\n`,
        );
        printTable(rows, ["category", "name", "status", "message"]);
        console.log("");

        if (
          report.contingenciesSummary &&
          report.contingenciesSummary.length > 0
        ) {
          console.log(
            `  🛡️  Active Contingency Plans & Fallback Strategies (${report.contingenciesSummary.length} active fallback(s)):\n`,
          );
          for (const item of report.contingenciesSummary) {
            ...
            if (item.remediationAction) {
              console.log(
                `  │   💡 Suggested Remedy: ${item.remediationAction}`,
              );
            }
            console.log(`  └──\n`);
          }
