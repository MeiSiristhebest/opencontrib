...
export async function readStdin(timeoutMs = 500): Promise<string> {
  if (process.stdin.isTTY) {
    throw new Error(
      "No input provided in interactive terminal. Pass --input <json> or pipe JSON via stdin.",
    );
  }
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      if (!hasData) {
        ...
        reject(
          new Error(
            "No input received on stdin. Pass --input <json> or pipe JSON via stdin.",
          ),
        );
      }
    }, timeoutMs);
...
export function printPhaseGuidance(options: PhaseGuidanceOptions): void {
  const protocolGuidance = options.currentPhase
    ? getProtocolGuidance(options.currentPhase)
    : undefined;
  const nextCommand = options.nextCommand ?? protocolGuidance?.cliExample;
  const forbiddenActions =
    options.forbiddenActions ?? protocolGuidance?.forbiddenActions;
  const invariants = options.invariants ?? protocolGuidance?.invariants;
  const statusEmoji =
...
export function printDefectCard(options: DefectCardOptions): void {
  ...
  console.log(
    "│" +
      " 🎯 SINGLE DEFECT SUMMARY CARD (RFC-100 ATOMIC GATE) ".padEnd(76) +
      "│",
  );
  console.log("├" + "─".repeat(76) + "┤");
  console.log(
    `│ 📍 Target File:   ${(options.file + ":" + options.line).padEnd(58)} │`,
  );
  console.log(`│ 🏷️  Category:      ${options.category.padEnd(58)} │`);
  console.log(
    `│ 💥 Core Defect:   ${options.summary.slice(0, 58).padEnd(58)} │`,
  );
  if (options.summary.length > 58) {
    console.log(
      `│                   ${options.summary.slice(58, 116).padEnd(58)} │`,
    );
  }
  if (options.reproCommand) {
    console.log(
      `│ 🧪 Repro Command: ${options.reproCommand.slice(0, 58).padEnd(58)} │`,
    );
  }
  if (options.fixScopeSnippet) {
    console.log(
      `│ 💡 Fix Scope:     ${options.fixScopeSnippet.slice(0, 58).padEnd(58)} │`,
    );
  }
  const note =
...
export function printCommunityGateAlert(
  options: CommunityGateAlertOptions,
): void {
  console.log("\n┌" + "─".repeat(76) + "┐");
  ...
  console.log(
    `│ 🚦 Gate Status:   ${(options.isPaused ? "🛑 HARD-PAUSED (Wait for Maintainer Approval)" : "⚠️ Active Review Rules").padEnd(58)} │`,
  );
